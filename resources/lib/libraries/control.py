...
scriptID = 'plugin.video.mrknow.wizjatv'
...
sessionFile = os.path.join(dataPath, 'session.pkl')
...
def save_session_obj(obj):
    with open(sessionFile, 'wb') as f:
        pickle.dump(obj, f, pickle.HIGHEST_PROTOCOL)

def load_session_obj():
    with open(sessionFile, 'rb') as f:
        return pickle.load(f)

def clear_session_file():
    try:
        log('TRY: clear_session_file():%s' % sessionFile)

        if os.path.isfile(sessionFile):
            os.unlink(sessionFile)
            log('SUKCES: clear_session_file():%s' % sessionFile)
    except Exception, e:
        log('ERROR: clear_session_file():%s' % e)
